import type { NormalizePanas, PanaType } from '../types';
...
export const normalizePanas = (
  panas: PanaType[]
): { normalizedData: NormalizePanas; rootIds: string[] } => {
...
export const filterPanas = (
  panas: NormalizePanas,
  filterIds: string[]
): PanaType[] =>
  filterIds.map((panaId) => panas[panaId]).filter((pana) => !!pana);
export const getPanaById = (panas: NormalizePanas, panaId: string): PanaType =>
  panas[panaId];
